...
main(int argc, char *argv[])
{
   ...
   while(1){
     ...
     while(1){
      ...
      // ADDED CODE: USE SSCANF TO GET TWO INTEGERS ENTERED BY USER
      int left, right, sum; 
      sscanf(line, "%d %d", &left, &right); 

      printf("server: adding %d + %d\n", left, right); 

      sum = left + right; 
      sprintf(line, "%d", sum);     /* print back to line */ 
      //strcat(line, " ECHO");

      // send the echo line to client 
      n = write(newsock, line, MAX);

      printf("server: wrote n=%d bytes; sum=[%d]\n", n, sum);
