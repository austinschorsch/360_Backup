...
char input[128], *inputArr[] = {};               /* Contains user input and tokeninized strings */
char *myargv[] = {}, *file;
...
/**
  ...
  *
  * Note on forks: forks are used to create processes. Whichever process is created by a fork is the child
  *                process of the parent process. After a successful fork, both processes execute the next 
  *                instruction. In other words, after a fork:
  *                         (1). Parent process goes to kmode to execute corresponding kfork() function
  *                         (2). This creates a child process which obtains the same image as the original process
  *                         (3). The original process returns to 'pid=fork()' as the child process
  *                         (4). Then runs, exits kmode
  */ 
...
{    
	...
	{
		 myargv[i] = tcmd;
		 if ((strcmp(tcmd, ">") == 0) || (strcmp(tcmd, "<") == 0) || (strcmp(tcmd, ">>") == 0)) { loc = i; } // Locate the redirect symbol
		 tcmd = strtok(NULL, " ");
		 //printf("myargv[%d]=%s\n", i, myargv[i]); 
		 i++;
	}
	 ...
	 {
         //printf("myargv[loc+1] = %s\n", myargv[loc+1]);
		 //if (myargv[loc+1] == ">") { file = myargv[loc+2]; printf(">>: %s", file); } 		// '>>' symbol -- Meaning the spot after the location found is '>'
		 file = myargv[loc+1];
          ...
          *         Note: The entire purpose of redirection is to 'update' the read/write file descriptors
		  */ 
		 if (rd == 1) // '<'
		 { 		
				close(0);
		 ...
		 {
				close(1);                                       /* Ex: Changed file descriptor 1 to point to file, then outputs */
				open(file, O_WRONLY|O_CREAT, 0644);             /* will go to that file instead of to the screen.               */
		 }
		 else 	// '>>'
		 {
				close(1);
     ...
     /**
       * Once everything else has been handled, append to the pathnames whatever the command was 
       * and then call execve with that pathname and myargv. This will execute the pathname (if possible)
       * or it will move on to the next pathname.
       */
	 char temp[128]; 	
	 int iter = 0;
	 for (iter = 0; iter < pathCount; iter++)
	 {
		 ...
		 execve(temp, myargv, cpenv);   /* Executes the program by filename which must be binary exec */
	 } 
...
/**
  * Function: IOredir(char *line) : int
...
void showPath()
{
    ...
    /**
      ...
      * Note: The paths are split by the ':' character. This is the token. 
      */
    char *token;
    token = strtok(path, ":");
